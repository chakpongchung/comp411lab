/*
    ...
    * Detection of '0' should only be at the head, not for every number;
    * The order of operand matters. We cannot just push(pop() - pop()),
        because the deeper one is really the one being subtracted from
    * In 'get_next', the detection of number allows *p to move to next spot
        easily, but the operator one isn't automatic and requires manually
        advancing.
*/
